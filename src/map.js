...
function receiptFor(map) {
  return createHash("sha256").update(JSON.stringify(canonical(map))).digest("hex");
}
...
export function inspectMap(map) {
  ...
  return {
    findings,
    metrics,
    receipt: findings.length === 0 ? receiptFor(map) : null,
...
export async function loadMap(file) {
  ...
  try {
    ...
    throw new MapValidationError([
      finding("$", "invalid-json", `Could not parse JSON: ${error.message}`),
    ]);
  }
